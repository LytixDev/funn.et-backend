...
public class ChatServiceImpl implements ChatService {
    ...
    public Chat createChat(@NonNull User user, @NonNull Listing listing) {
        if (chatRepository.findChatsByMessager(user.getUsername()).stream().anyMatch(chat -> chat.getListing().getId() == listing.getId()))
        ...
        return chatRepository.save(Chat.builder().messager(user).listing(listing).messages(new HashSet<>()).build());
    }
    ...
    public Chat getChat(@NonNull User user, @NonNull Long id) {
        Chat chat = chatRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Chat does not exist"));

        if (!chat.getMessager().getUsername().equals(user.getUsername()) || !chat.getListing().getUser().getUsername().equals(user.getUsername()))
    ...
    public void sendMessage(@NonNull User user, @NonNull Chat chat, @NonNull String message) {
        ...
        chat.getMessages().add(Message.builder().chat(chat).message(message).sender(user).timestamp(new Timestamp(System.currentTimeMillis())).build());
