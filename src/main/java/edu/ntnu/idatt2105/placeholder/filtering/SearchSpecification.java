...
public class SearchSpecification<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        Predicate predicate = builder.equal(builder.literal(Boolean.TRUE), Boolean.TRUE);

        for (FilterRequest filter : this.searchRequest.getFilterRequests())
            predicate = filter.getOperator().build(root, builder, filter, predicate);

        List<Order> orders = new ArrayList<>();
        for (SortRequest sort : this.searchRequest.getSortRequests())
            orders.add(sort.getSortDirection().build(root, builder, sort));
    ...
    public static Pageable getPageable(SearchRequest searchRequest) {
        return PageRequest.of(searchRequest.getPage() > 0 ? searchRequest.getPage() : 0, searchRequest.getSize() > 0 ? searchRequest.getSize() : 20);
    }
}
