  ...
  GREATER_THAN {
    public <T> Predicate build(
      ...
      if (request.getFieldType() == FieldType.DATE) {
        ...
        return builder.and(
          builder.greaterThan(key, (LocalDate) value),
          predicate
        );
      }
  ...
  GREATER_THAN_EQUAL {
    public <T> Predicate build(
      ...
      if (request.getFieldType() == FieldType.DATE) {
        Expression<LocalDate> key = root.get(request.getKeyWord());
        return builder.and(
          builder.greaterThanOrEqualTo(key, (LocalDate) value),
          predicate
        );
      }
  ...
  LESS_THAN_EQUAL {
    public <T> Predicate build(
      ...
      if (request.getFieldType() == FieldType.DATE) {
        Expression<LocalDate> key = root.get(request.getKeyWord());
        return builder.and(
          builder.lessThanOrEqualTo(key, (LocalDate) value),
          predicate
        );
      }
  ...
  BETWEEN {
    public <T> Predicate build(
      ...
      Object value = request
        .getFieldType()
        .parse(request.getValue().toString());
      Object valueTo = request
        .getFieldType()
        .parse(request.getValueTo().toString());

      if (request.getFieldType() == FieldType.DATE) {
        Expression<LocalDate> key = root.get(request.getKeyWord());
        return builder.and(
          builder.between(key, (LocalDate) value, (LocalDate) valueTo),
          predicate
        );
      }
      ...
      if (
        ...
        return builder.and(
          builder.and(
            builder.ge(key, (Number) value),
            builder.le(key, (Number) valueTo)
          ),
          predicate
        );
      }
