...
public enum Operator {
    ...
    GREATER_THAN {
        public <T> Predicate build(Root<T> root, CriteriaBuilder builder, FilterRequest request, Predicate predicate) {
            ...
            if (request.getFieldType() != FieldType.CHARACTER && request.getFieldType() != FieldType.BOOLEAN && request.getFieldType() != FieldType.STRING) {
    ...
    BETWEEN {
        public <T> Predicate build(Root<T> root, CriteriaBuilder builder, FilterRequest request, Predicate predicate) {
            ...
            if (request.getFieldType() != FieldType.CHARACTER && request.getFieldType() != FieldType.BOOLEAN && request.getFieldType() != FieldType.STRING) {
                Expression<Number> key = root.get(request.getKeyWord());
                return builder.and(builder.and(builder.ge(key, (Number) value), builder.le(key, (Number) valueTo)), predicate);
            }
    ...
    LIKE {
        public <T> Predicate build(Root<T> root, CriteriaBuilder builder, FilterRequest request, Predicate predicate) {
            ...
            return builder.and(builder.like(builder.upper(key), "%" + request.getValue().toString().toUpperCase() + "%"), predicate);
        }
    };
