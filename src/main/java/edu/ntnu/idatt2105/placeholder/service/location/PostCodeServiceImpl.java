...
public class PostCodeServiceImpl implements PostCodeService {
    ...
    public boolean postCodeExists(PostCode postcode) {
        return postCodeRepository.existsById(new PostCodeId(postcode.getPostCode(), postcode.getCity()));
    }

    @Override
    public PostCode savePostCode(PostCode postcode) throws PostCodeAlreadyExistsException, DatabaseException {
        if (postCodeExists(postcode))
            throw new PostCodeAlreadyExistsException("Postcode already exists");
    ...
    @Override
    public PostCode updatePostCode(@NotNull PostCode postcode) throws PostCodeDoesntExistException, DatabaseException, NullPointerException {
        if (!postCodeExists(postcode))
            throw new PostCodeDoesntExistException("Postcode does not exist");
    ...
    @Override
    public void deletePostCode(PostCode postcode) throws PostCodeDoesntExistException, DatabaseException {
        if (!postCodeExists(postcode))
            throw new PostCodeDoesntExistException("Postcode does not exist");
    ...
    @Override
    public List<String> getCitiesByPostCode(String postcode) throws DatabaseException {
        return postCodeRepository.findCitiesByPostCode(postcode).orElseThrow(() -> new DatabaseException("No cities found for postcode."));
    }

    @Override
    public List<String> getPostCodesByCity(String city) throws DatabaseException {
        return postCodeRepository.findPostCodesByCity(city).orElseThrow(() -> new DatabaseException("No postcodes found for city found."));
    }
}
